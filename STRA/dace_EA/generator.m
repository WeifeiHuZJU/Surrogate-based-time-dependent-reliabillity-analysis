function u = generator(p, lu, i, F, CR, popsize, n, paraIndex,bestind)
...
if rand<0.5
    ...
    v1 = bestind + F(paraIndex(1)) .* (p(index(2), :) - p(index(3), :));
end
...
if rand<0.5
    ...
    v2 = p(i, :) + rand * (bestind - p(i, :)) + F(paraIndex(2)) .* (p(index(2), :) - p(index(3), :));
end
...
if rand<0.5
    ...
    v3 = p(index(1), :) + rand .* (bestind - p(index(3), :)) + F(paraIndex(3)) .* (p(index(4), :) - p(index(5), :));
end
