function  [dmodel, perf] = dacefit(S, Y, regr, corr, theta0, lob, upb,Using_EA)
global par;
...
lth = length(theta0);
if  nargin > 5 && Using_EA==0  % optimization case
...
for k = 1 : m-1
  ll = ll(end) + (1 : m-k);
  ij(ll,:) = [repmat(k, m-k, 1) (k+1 : m)']; % indices for sparse matrix  ij对应的就是 D矩阵中对应的个体的差值，与D具有相同的行，也就是D矩阵的第一行的值就是对应的 ij 矩阵中第一行中第一列的值的个体（也就是第一个个体）与地一行第二列的个体（也就是第499个个体）之间的差值。
  D(ll,:) = repmat(S(k,:), m-k, 1) - S(k+1:m,:); % differences between points  如果样本点数为 m，维度为k，则 D是一个 m(m-1)/2 * k 的矩阵
  % D 矩阵记录的是相关系数矩阵中下三角矩阵的所有相关系数值对应的个体的差的列向量，所以，如果样本点为500.相关系数矩阵中第一列有499个，第二列有498个，直到最后第499列有1个，这些按照顺序排成一个列向量就是一共有（499+498+497+。。。+1）行就是 124750行。针对一个 m 个样本点数建模，则这个D矩阵就是 m(m-1)/2行。
end
...
%   theta = theta0(:);   
  ...
  [~,theta]=de_call(lob, upb, par);
%   [theta,~]=JADE_test(lob, upb, par);
  theta=theta';
    
%   xlb=lob;
%   xub=upb;
%   x0=10*ones(1,n);
%   options = optimset('Algorithm','interior-point', 'MaxFunEvals', 300, 'Display', 'on');
%   theta = fmincon(@objfunc_grad, x0, [], [], [], [], xlb', xub', [], options);
%   theta=theta';
...
function  obj = objfunc_grad(theta)
global par;
% Initialize
obj = inf; 
